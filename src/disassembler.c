...
uint32_t videobuf[64 * 32]; // 64 pixels wide and 32 pixels high (32-bit to make
                            // using with SDL easier)
    ...
    for (unsigned int row = 0; row < height; ++row) {
      ...
      for (unsigned int col = 0; col < 8; ++col) {
        ...
        uint32_t *screenPixel =
            &videobuf[(yPos + row) * VIDEO_WIDTH + (xPos + col)];
...
uint8_t randbyte() { return (uint8_t)(rand() % 256); }
